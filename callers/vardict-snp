...
echo -e "CHROM\tPOS\tREF\tALT\tTYPE\tDP\tVD\tAF\tQUAL\tQSTD\tMQ\tSN\tHIAF\tADJAF\tNM\tHICNT\tHICOV\tSVTYPE\tSVLEN\tDUPRATE\tGT" > "$output_dir/vardict-snp.tsv"
vcftools --vcf "$output_dir/vardict.vcf" --remove-indels --recode --recode-INFO-all -c | \
bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%INFO/TYPE\tI%NFO/DP\t%INFO/VD\t%INFO/AF\t%INFO/QUAL\t%INFO/QSTD\t%INFO/MQ\t%INFO/SN\t%INFO/HIAF\t%INFO/ADJAF\t%INFO/NM\t%INFO/HICNT\t%INFO/HICOV\t%INFO/SVTYPE\t%INFO/SVLEN\t%INFO/DUPRATE\t[%GT]\n' - >> "$output_dir/vardict-snp.tsv"
